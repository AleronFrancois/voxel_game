...
impl Default for FpsCounter {
    fn default() -> Self {
        Self {
            timer: Timer::new(config::UPDATE_INTERVAL, TimerMode::Repeating),
            update_now: true,
        }
    }
}
...
fn update(
    ...
    if state.timer.is_paused() {
        ...
        let fps_value = extract_fps(&diagnostics);

        for entity in query.iter_mut() {
            if let Some(fps) = fps_value {
                *writer.text(entity, 0) = format!("{}{:.0}", config::STRING_FORMAT, fps);
            } 
            else {
                *writer.text(entity, 0) = config::STRING_MISSING.to_string();
            }
        }
    }
}
